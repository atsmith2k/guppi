...
- `guppi symbol <name>`: Serena-style scope-aware symbol search enriched with memory.
...
- `guppi://recipes/codebase_traversal`: Decision matrix for Serena-style symbol navigation.
...
- `guppi_onboard`: Trigger workspace scan & AST indexing.
...
### 🛡️ Pillar 4: Safety, Mutation & Self-Healing ([`safety_pillar.ts`](file:///Users/ashton/git/something/src/server/pillars/safety_pillar.ts))
...
- [src/server/pillars/](file:///Users/ashton/git/something/src/server/pillars/): 6 modular tool & resource registration pillars (`context_pillar`, `memory_pillar`, `planning_pillar`, `safety_pillar`, `quality_pillar`, `lst_pillar`).
- [src/engine/lst_traversal.ts](file:///Users/ashton/git/something/src/engine/lst_traversal.ts): Serena-style Lossless Semantic Tree (LST) Engine & Reference Tracer.
